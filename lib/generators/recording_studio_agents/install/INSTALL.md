...
5. Register skills, knowledge, skill packs, and agents, then call Agent#run from a controller or job. Each knowledge entry must cite a source recording inside the task root. Required skills always compile. Pass pack: or extra_skills: only when a ticket needs optional skills. Show coarse steps with Progress.for(run).
