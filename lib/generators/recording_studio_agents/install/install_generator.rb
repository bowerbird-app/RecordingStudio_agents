...
module RecordingStudioAgents
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs RecordingStudioAgents engine into your application"

      class_option(
        :mount_path,
        type: :string,
        default: "/recording_studio_agents",
        desc: "Route prefix used when mounting the engine"
      )

      def mount_engine
        route %(mount RecordingStudioAgents::Engine, at: "#{options[:mount_path]}")
      end

      def copy_initializer
        template "recording_studio_agents_initializer.rb", "config/initializers/recording_studio_agents.rb"
      end

      def add_yaml_config
        unless yes?("Would you like to add `config/recording_studio_agents.yml` for environment-specific settings? [y/N]")
          return
        end

        template "recording_studio_agents.yml", "config/recording_studio_agents.yml"
      end
      ...
      def add_tailwind_source
        ...
        if missing_lines.empty?
          say "Tailwind already configured to include RecordingStudioAgents and FlatPack sources.", :green
          return
        end
      ...
      def inject_tailwind_sources(tailwind_css_path, missing_lines)
        ...
        say "Added RecordingStudioAgents and FlatPack sources to Tailwind CSS configuration.", :green
        say "Run 'bin/rails tailwindcss:build' to rebuild your CSS.", :green
      end

      def formatted_tailwind_source_block(missing_lines)
        [
          "\n/* Include RecordingStudioAgents engine views for Tailwind CSS */",
      ...
      def tailwind_source_lines
        [
          '@source "../../vendor/bundle/**/recording_studio_agents/app/views/**/*.erb";',
          '@source "../../../../../../usr/local/bundle/ruby/**/bundler/gems/recording_studio_agents-*/app/views/**/*.erb";',
