...
**Engine Isolation**: Respect namespace isolation. All code should be properly namespaced under the engine's module (`RecordingStudioAgents`). Use `isolate_namespace` patterns correctly.
...
**Engine-Specific**:
- Prefix all database tables with the engine name (e.g., `recording_studio_agents_agent_runs`)
...
- **Browser Simulation**: When modifying UI or flows (like `dashboard/index.html.erb`), explicitly verify the user experience:
  ...
  2. Navigate to the relevant page (e.g., `/` or `/admin`).
...
When generating code, include file paths in comments (e.g., `# app/models/recording_studio_agents/agent_run.rb`).
...
**Configuration**:
- Provide a configuration block: `RecordingStudioAgents.configure do |config|`
