...
The dummy has a Playground page at `/playground`. The form sits on the left and the steps on the right. The right side is blank until a run has steps. Each step is a collapse with a title and a progress badge. Open a tool step to see the data that was passed in. The reply is a collapse when the run finishes. Pick a registered agent, write an instruction, and search for the tools and skills that run may use. Page librarian can list the workspace pages and the menu pages (Home, Playground, Staff, and Agents) before it looks one up by title. The dummy keeps model replies so that page can show the text. Admin model calls include workspace runs, so that call is listed.
