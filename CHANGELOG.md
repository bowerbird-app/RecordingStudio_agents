...
### Added
- Code registries for skills, knowledge, and agents, compiled into an immutable program
- `Agent#run` with a required `idempotency_key`, lease, and adoption of an existing Recording Studio AI run
- Task, agent run, activity, and evaluation tables. Runs store `output_digest` and an indexed `recording_studio_ai_run_id` with no foreign key. They do not copy model output.
