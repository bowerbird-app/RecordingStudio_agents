...
Admin lists every skill and opens its details from the name.

### Added
- The Agents admin section links to Skills. The table lists each skill's name and version. The name opens that skill's details. The registry key is an optional column.

### Upgrade notes
- No migration. The list is `/admin/screens/registered_skills`. Skill packs stay in code.
