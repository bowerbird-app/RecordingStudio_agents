...
### Changed
- Dummy playground results start blank, including when a new run starts. Each step is a collapse with a title and a progress badge. The reply is a collapse when the run finishes. The agent name, skills, and model-call link are not repeated on that side.
