...
- Playground at `/playground`. The form sits on the left. The right side stays blank until a run has steps. Each step is a collapse with a title and a Working, Waiting, Done, or Failed badge. Open a step for what it did. A tool step also shows the data that was passed in. When the run finishes, the reply is a collapse too. Model replies are kept. Admin lists model calls from workspaces as well as the staff root. The dummy sets fixture Active Record encryption keys when the host has none, so that reply can be read.
