...
- Playground at `/playground`. The form sits on the left. The right side stays blank until a run has model turns. While a run is going, each turn shows up when that turn starts, and the list updates in place. Each turn is a collapse with a title and a Working, Waiting, Done, or Failed badge. Open a turn to see the input that turn received and the response it returned. A later turn is given the tool result, not the original instruction again. Model replies are kept. Admin lists model calls from workspaces as well as the staff root. The dummy sets fixture Active Record encryption keys when the host has none, so that reply can be read.
