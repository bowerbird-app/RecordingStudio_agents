...
class CursorSkillFetchTest < ActiveSupport::TestCase
  test "fetch-skills extras come from the plugin catalog without hardcoded extra URLs" do
    root = RecordingStudioAgents::Engine.root
  ...
  test "gem version stays current and gemspec still excludes .cursor" do
    assert_equal "0.4.10", RecordingStudioAgents::VERSION
    ...
    tracked = `git -C #{RecordingStudioAgents::Engine.root} ls-files -- .cursor/rules`
